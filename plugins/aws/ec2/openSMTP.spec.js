...
const openSMTP = require('./openSMTP');
...
describe('openSMTP', function () {
    describe('run', function () {
        it('should PASS if no public open ports found', function (done) {
            ...
            openSMTP.run(cache, {}, (err, results) => {
        ...
        it('should FAIL if security group has SMTP TCP port open to public', function (done) {
        ...
        it('should WARN if security group is unused', function (done) {
            ...
            openSMTP.run(cache, {ec2_skip_unused_groups: 'true'}, (err, results) => {
                expect(results.length).to.equal(1);
        ...
        it('should PASS if open port security group attached to the network interface has no public IP associated', function (done) {
            const cache = createCache([describeSecurityGroups[1]], [describeNetworkInterfaces[0]], [listFunctions[0]]);
            openSMTP.run(cache, {check_network_interface:'true'}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                done();
            });
