...
var acmValidation = require('./acmValidation');
...
describe('acmValidation', function () {
    describe('run', function () {
        it('should PASS if ACM certificate is using DNS validations', function (done) {
            const cache = createCache([listCertificates[0]], null, describeCertificate[0]);
            acmValidation.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].region).to.equal('us-east-1');
                expect(results[0].message).to.include('is using DNS validation')
                done();
            });
        });

        it('should FAIL if ACM certificate has failed validations', function (done) {
            const cache = createCache([listCertificates[0]], null, describeCertificate[1]);
            acmValidation.run(cache, { acm_certificate_expiry_warn: '35' }, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].region).to.equal('us-east-1');
                expect(results[0].message).to.include('has failed DNS validation')
                done();
            });
        });

        it('should WARN if ACM certificate is using EMAIL validation', function (done) {
            const cache = createCache([listCertificates[0]], null, describeCertificate[2]);
            acmValidation.run(cache, { acm_certificate_expiry_warn: '35' }, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(1);
                expect(results[0].region).to.equal('us-east-1');
                expect(results[0].message).to.include('is using EMAIL validation')
                done();
            });
        });
