...
const elbHttpsOnly = require('./elbHttpsOnly');
...
describe('elbHttpsOnly', function () {
    describe('run', function () {
        it('should PASS if ELB is using listeners with HTTPS protocol only', function (done) {
            const cache = createCache([describeLoadBalancers[0]]);
            elbHttpsOnly.run(cache, {}, (err, results) => {
        ...
        it('should FAIL if ELB is using listeners without HTTPS protocol', function (done) {
        ...
        it('should PASS if ELB is not using any listeners', function (done) {
            const cache = createCache([describeLoadBalancers[2]]);
            elbHttpsOnly.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                done();
