...
describe('appTierIamRole', function () {
    describe('run', function () {
        it('should PASS if launch configuration for App-Tier group has customer IAM role configured', function (done) {
            const cache = createCache([describeAutoScalingGroups[0]], [describeLaunchConfigurations[0]]);
            appTierIamRole.run(cache, { app_tier_tag_key: 'app_tier' }, (err, results) => {
        ...
        it('should FAIL if launch configuration for App-Tier group does not have customer IAM role configured', function (done) {
