...
const accessKeysExtra = require('./accessKeysExtra');
...
describe('accessKeysExtra', function () {
    describe('run', function () {
        it('should PASS if user is not using both access keys', function (done) {
            const cache = createCache([generateCredentialReport[0],generateCredentialReport[1],generateCredentialReport[1]]);
            accessKeysExtra.run(cache, {}, (err, results) => {
        ...
        it('should FAIL if user is using both access keys', function (done) {
            const cache = createCache([generateCredentialReport[0],generateCredentialReport[1],generateCredentialReport[2]]);
            accessKeysExtra.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(2);
                expect(results[0].status).to.equal(0);
        ...
        it('should PASS if no user using access keys found', function (done) {
        ...
        it('should PASS if no users using both access keys found', function (done) {
            const cache = createCache([generateCredentialReport[0],generateCredentialReport[0],generateCredentialReport[0]]);
            accessKeysExtra.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                done();
