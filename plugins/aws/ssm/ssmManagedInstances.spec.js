...
describe('ssmManagedInstances', function () {
    describe('run', function () {
        ...
        it('should PASS if EC2 instance is being managed by AWS SSM', function (done) {
            const cache = createCache([describeInstances[0]], [describeInstanceInformation[0]]);
            ssmManagedInstances.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                done();
        ...
        it('should PASS if EC2 instance is not in the Running State', function (done) {
            const cache = createCache([describeInstances[1]], [describeInstanceInformation[0]]);
            ssmManagedInstances.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                done();
