...
var dataCatalogEncryptionEnabled = require('./dataCatalogEncryptionEnabled');
...
describe('dataCatalogEncryptionEnabled', function () {
    describe('run', function () {
        it('should PASS if Glue data catalog has encryption at-rest enabled for metadata at encryption level greater than or equal to target encryption level', function (done) {
            const cache = createCache(getDataCatalogEncryptionSettings[0], describeKey[0]);
            dataCatalogEncryptionEnabled.run(cache, {}, (err, results) => {
        ...
        it('should FAIL if Glue data catalog has encryption at-rest enabled for metadata at encryption level less than target encryption level', function (done) {
            const cache = createCache(getDataCatalogEncryptionSettings[0], describeKey[1]);
            dataCatalogEncryptionEnabled.run(cache, { glue_datacatalog_encryption_level: 'awscmk'}, (err, results) => {
                expect(results.length).to.equal(1);
