...
var plugin = require('./policyLeastPrivilege');
...
describe('policyLeastPrivilege', function () {
    describe('run', function () {
        ...
        it('should give failing result if a non service-level admin has full permissions', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
                expect(results[0].message).to.include('has the ability to ')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should give passing result if only service-level admin have full permissions', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(0)
                expect(results[0].message).to.include('All policies follow least access')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
