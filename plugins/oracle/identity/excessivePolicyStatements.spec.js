...
var plugin = require('./excessivePolicyStatements');
...
describe('excessivePolicyStatements', function () {
    describe('run', function () {
        ...
        it('should give warning result if there are an excessive amount of policy Statements', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(1)
                expect(results[0].message).to.include('number of policy statements')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should give failing result if there are an excessive amount of policies', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
        ...
        it('should give passing result if there do not have excessive amount of policy Statements', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(0)
                expect(results[0].message).to.include('number of policy statements')
