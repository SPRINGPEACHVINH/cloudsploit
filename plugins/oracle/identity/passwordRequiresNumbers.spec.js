...
var plugin = require('./passwordRequiresNumbers');
...
describe('passwordRequiresNumbers', function () {
    describe('run', function () {
        ...
        it('should give passing result if password policy requires numeric characters', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(0)
                expect(results[0].message).to.include('Password policy requires numeric characters')
                expect(results[0].region).to.equal('global')
                done()
            };
        ...
        it('should give warn result if password policy does not require numeric characters', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(1)
                expect(results[0].message).to.include('Password policy does not require numeric characters')
                expect(results[0].region).to.equal('global')
                done()
            };
