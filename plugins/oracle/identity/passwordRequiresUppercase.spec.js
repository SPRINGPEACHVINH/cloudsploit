...
var plugin = require('./passwordRequiresUppercase');
...
describe('passwordRequiresUppercase', function () {
    describe('run', function () {
        ...
        it('should give passing result if password policy requires uppercase characters', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(0)
                expect(results[0].message).to.include('Password policy requires uppercase characters')
                expect(results[0].region).to.equal('global')
                done()
            };
        ...
        it('should give warn result if password policy does not require uppercase characters', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(1)
                expect(results[0].message).to.include('Password policy does not require uppercase characters')
                expect(results[0].region).to.equal('global')
                done()
            };
