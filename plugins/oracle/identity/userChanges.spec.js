...
var plugin = require('./userChanges');
...
const rules = [
    {
        ...
        condition: '{"eventType":["com.oraclecloud.identitycontrolplane.createuser","com.oraclecloud.identitycontrolplane.updateuser","com.oraclecloud.identitycontrolplane.deleteuser", "com.oraclecloud.identitycontrolplane.updateuserstate", "com.oraclecloud.identitycontrolplane.updateusercapabilities"],"data":{}}',
    ...
    },
    {
        ...
        displayName: 'Rule 1',
        lifecycleState: 'ACTIVE',
        condition: '{"eventType":["com.oraclecloud.identitycontrolplane.createuser","com.oraclecloud.identitycontrolplane.updateuser","com.oraclecloud.identitycontrolplane.deleteuser"],"data":{}}',
        compartmentId: 'ocid1.tenancy.oc1.111111111',
        isEnabled: true,
        timeCreated: '2022-07-03T23:57:52.769Z',
    },
...
describe('userChanges', function () {
    describe('run', function () {
        ...
        it('should give failing result if no user rules are configured', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
        ...
        it('should give failing result if Event rules are missing for some user changes', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
        ...
        it('should give passing result if event rules are configured for all user changes', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
