...
var plugin = require('./passwordRequiresLowercase');
...
describe('passwordRequiresLowercase', function () {
    describe('run', function () {
        ...
        it('should give passing result if password policy requires lowercase characters', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(0)
                expect(results[0].message).to.include('Password policy requires lowercase characters')
        ...
        it('should give warn result if password policy does not require lowercase characters', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(1)
                expect(results[0].message).to.include('Password policy does not require lowercase characters')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
