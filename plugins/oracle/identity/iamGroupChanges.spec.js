...
var plugin = require('./iamGroupChanges');
...
const rules = [
    {
        ...
        condition: '{"eventType":["com.oraclecloud.identitycontrolplane.creategroup","com.oraclecloud.identitycontrolplane.updategroup","com.oraclecloud.identitycontrolplane.deletegroup"],"data":{}}',
    ...
    },
    {
        ...
        displayName: 'Rule 1',
        lifecycleState: 'ACTIVE',
        condition: '{"eventType":["com.oraclecloud.identitycontrolplane.creategroup"],"data":{}}',
        compartmentId: 'ocid1.tenancy.oc1.111111111',
        isEnabled: true,
        timeCreated: '2022-07-03T23:57:52.769Z',
    },
...
describe('iamGroupChanges', function () {
    describe('run', function () {
        ...
        it('should give failing result if no IAM group rules are configured', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
        ...
        it('should give failing result if Event rules are missing for some IAM Group changes', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
        ...
        it('should give passing result if event rules are configured for all IAM Group changes', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
