...
var plugin = require('./identityProviderChanges');
...
const rules = [
    {
        ...
        condition: '{"eventType":["com.oraclecloud.identitycontrolplane.createidentityprovider","com.oraclecloud.identitycontrolplane.updateidentityprovider","com.oraclecloud.identitycontrolplane.deleteidentityprovider"],"data":{}}',
    ...
    },
    {
        ...
        displayName: 'Rule 1',
        lifecycleState: 'ACTIVE',
        condition: '{"eventType":["com.oraclecloud.identitycontrolplane.deleteidentityprovider"],"data":{}}',
        compartmentId: 'ocid1.tenancy.oc1.111111111',
        isEnabled: true,
        timeCreated: '2022-07-03T23:57:52.769Z',
    },
...
describe('identityProviderChanges', function () {
    describe('run', function () {
        ...
        it('should give failing result if no identity provider rules are configured', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
        ...
        it('should give failing result if Event rules are missing for some identity provider changes', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
        ...
        it('should give passing result if event rules are configured for all identity provider changes', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
