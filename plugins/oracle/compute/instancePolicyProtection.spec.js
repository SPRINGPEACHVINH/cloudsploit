...
var plugin = require('./instancePolicyProtection');
...
describe('instancePolicyProtection', function () {
    describe('run', function () {
        ...
        it('should give failing result if a policy does not have compute instance deletion protection', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
        ...
        it('should give passing result if a policy has compute instance deletion protection enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(0)
                expect(results[0].message).to.include('All policies have compute instance delete protection enabled')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
