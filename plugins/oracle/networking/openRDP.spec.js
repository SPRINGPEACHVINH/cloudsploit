...
describe('openRDP', function () {
    describe('run', function () {
        ...
        it('should FAIL if security list has RDP port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
                expect(results[0].message).to.include('has RDP: TCP port 3389 open to')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should FAIL if network security group has RDP port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[1].status).to.equal(2)
                expect(results[1].message).to.include('has RDP: TCP port 3389 open to')
