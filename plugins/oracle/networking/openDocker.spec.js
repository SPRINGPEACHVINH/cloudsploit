...
const plugin = require('./openDocker');
...
const networkSecurityGroups = [
    {
      ...
      "definedTags": {
        "Oracle-Tags": {
          ...
          "CreatedOn": "2022-01-09T13:03:47.837Z"
        }
      },
...
describe('openDocker', function () {
    describe('run', function () {
        ...
        it('should FAIL if security list has Docker port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
                expect(results[0].message).to.include('has Docker: TCP port 2375,2376 open to')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should FAIL if network security group has Docker port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[1].status).to.equal(2)
                expect(results[1].message).to.include('has Docker: TCP port 2375,2376 open to')
