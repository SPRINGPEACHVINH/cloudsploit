...
const plugin = require('./openSQLServer');
...
describe('openSQLServer', function () {
    describe('run', function () {
        ...
        it('should FAIL if security list has SQL Server port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
                expect(results[0].message).to.include('has SQL Server: UDP port 1434 open to')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should FAIL if network security group has SQL Server port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[1].status).to.equal(2)
                expect(results[1].message).to.include('has SQL Server: TCP port 1433 open to')
