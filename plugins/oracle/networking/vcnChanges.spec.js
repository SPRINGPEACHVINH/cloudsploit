...
const rules = [
    {
        ...
        condition: '{"eventType":["com.oraclecloud.virtualnetwork.createvcn","com.oraclecloud.virtualnetwork.updatevcn","com.oraclecloud.virtualnetwork.deletevcn", "com.oraclecloud.virtualnetwork.updatevcnstate", "com.oraclecloud.virtualnetwork.updatevcncapabilities"],"data":{}}',
    ...
    },
    {
        ...
        displayName: 'Rule 1',
        lifecycleState: 'ACTIVE',
        condition: '{"eventType":["com.oraclecloud.virtualnetwork.createvcn","com.oraclecloud.virtualnetwork.updatevcn"],"data":{}}',
        compartmentId: 'ocid1.tenancy.oc1.111111111',
        isEnabled: true,
        timeCreated: '2022-07-03T23:57:52.769Z',
    },
...
describe('vcnChanges', function () {
    describe('run', function () {
        ...
        it('should give failing result if no vcn rules are configured', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
        ...
        it('should give failing result if Event rules are missing for some vcn changes', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
        ...
        it('should give passing result if event rules are configured for all vcn changes', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
