...
const plugin = require('./openVNCServer');
...
const securityLists = [
    ...
    },
    {
      ...
      "id": "list2",
      "lifecycleState": "AVAILABLE",
      "timeCreated": "2022-01-09T13:02:20.422Z",
      "vcnId": "vcn1"
    }
  ];
...
describe('openVNCServer', function () {
    describe('run', function () {
        ...
        it('should FAIL if security list has VNC Server port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
                expect(results[0].message).to.include('has VNC Server: TCP port 5900 open to')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should FAIL if network security group has VNC Server port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[1].status).to.equal(2)
                expect(results[1].message).to.include('has VNC Server: TCP port 5900 open to')
