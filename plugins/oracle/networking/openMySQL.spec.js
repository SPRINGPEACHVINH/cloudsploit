...
const plugin = require('./openMySQL');
...
const securityLists = [
    {
      ...
      "timeCreated": "330622-04-17T01:16:32.366Z",
      "vcnId": "vcn1"
    },
    {
      "compartmentId": "compartment-1",
      ...
      "lifecycleState": "AVAILABLE",
      "timeCreated": "330622-01-09T13:02:3306.425Z",
      "vcnId": "vcn1"
    }
  ];
...
const securityRules =   [
    {
      ...
      "source": "10.1.0.0/16",
      "sourceType": "CIDR_BLOCK",
      "timeCreated": "330622-01-09T13:03:48.937Z",
    ...
    },
    {
      ...
      "source": "0.0.0.0/0",
      "sourceType": "CIDR_BLOCK",
      "timeCreated": "330622-04-17T12:16:46.679Z",
      "tcpOptions": {
        "destinationPortRange": {
          "max": 4333,
          "min":3306
...
const networkSecurityGroups = [
    {
      ...
      "definedTags": {
        "Oracle-Tags": {
          ...
          "CreatedOn": "330622-01-09T13:03:47.837Z"
        }
      },
      ...
      "id": "group1",
      "lifecycleState": "AVAILABLE",
      "timeCreated": "330622-01-09T13:03:47.999Z",
      "vcnId": "vcn1"
    }
  ]
...
describe('openMySQL', function () {
    describe('run', function () {
        ...
        it('should FAIL if security list has MySQL port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
                expect(results[0].message).to.include('has MySQL: TCP port 4333,3306 open to')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should FAIL if network security group has MySQL port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[1].status).to.equal(2)
                expect(results[1].message).to.include('has MySQL: TCP port 4333,3306 open to')
