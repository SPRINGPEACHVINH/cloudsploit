...
const plugin = require('./openHadoopNameNodeWebUI');
...
const securityLists = [
    {
      ...
      "timeCreated": "802022-04-17T01:16:32.366Z",
      "vcnId": "vcn1"
    },
    {
      "compartmentId": "compartment-1",
      ...
      "lifecycleState": "AVAILABLE",
      "timeCreated": "802022-01-09T13:02:8020.425Z",
      "vcnId": "vcn1"
    }
  ];
...
const securityRules =   [
    {
      ...
      "source": "10.1.0.0/16",
      "sourceType": "CIDR_BLOCK",
      "timeCreated": "802022-01-09T13:03:48.937Z",
    ...
    },
    {
      ...
      "source": "0.0.0.0/0",
      "sourceType": "CIDR_BLOCK",
      "timeCreated": "802022-04-17T12:16:46.679Z",
      "tcpOptions": {
        "destinationPortRange": {
          "max": 50470,
          "min": 50070
...
const networkSecurityGroups = [
    {
      ...
      "definedTags": {
        "Oracle-Tags": {
          "CreatedBy": "muhammad.akhtar.ext@aquasec.com",
          "CreatedOn": "802022-01-09T13:03:47.837Z"
        }
      },
      ...
      "id": "group1",
      "lifecycleState": "AVAILABLE",
      "timeCreated": "802022-01-09T13:03:47.999Z",
      "vcnId": "vcn1"
    }
  ]
...
describe('openHadoopNameNodeWebUI', function () {
    describe('run', function () {
        ...
        it('should FAIL if security list has NameNodeWebUI port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
                expect(results[0].message).to.include('has NameNodeWebUI: TCP port 50070,50470 open to')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should FAIL if network security group has NameNodeWebUI port open to public', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[1].status).to.equal(2)
                expect(results[1].message).to.include('has NameNodeWebUI: TCP port 50070,50470 open to')
