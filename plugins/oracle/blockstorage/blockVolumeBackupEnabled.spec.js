...
var plugin = require('./blockVolumeBackupEnabled');
...
describe('blockVolumeBackupEnabled', function () {
    describe('run', function () {
        it('should give unknown result if a volume error is passed or no data is present', function (done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('Unable to query for block volumes');
        ...
        it('should give unknown result if a volume backup error is passed or no data is present', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(1)
                expect(results[0].status).to.equal(3)
                expect(results[0].message).to.include('Unable to query for block volume backups')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should give failing result if there is a block volume without a backup', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(1)
                expect(results[0].status).to.equal(2)
                expect(results[0].message).to.include('Block volume has a backup policy disabled')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
                ...
                [
                    {
                        ...
                        "uniqueSizeInGBs": 1,
                        "uniqueSizeInMbs": 1,
                        "assetId": "ocid1.volume.oc1.iad.abuwcljtquhbwu5divro64gimkrnfdaxo43cy44cbpuz42g652ol4gw6qsma"
                    }
                ],
                null
            );
        ...
        it('should give passing result if all block volumes have backups', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(1)
                expect(results[0].status).to.equal(0)
                expect(results[0].message).to.include('Block volume has a backup policy enabled')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
