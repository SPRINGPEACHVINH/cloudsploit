...
var plugin = require('./dbPolicyProtection');
...
describe('dbPolicyProtection', function () {
    describe('run', function () {
        ...
        it('should give failing result if a policy does not have database deletion protection', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
        ...
        it('should give passing result if a policy has database deletion protection enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(0)
