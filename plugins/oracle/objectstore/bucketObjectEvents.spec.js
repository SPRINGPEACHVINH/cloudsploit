...
var plugin = require('./bucketObjectEvents');
...
describe('bucketObjectEvents', function () {
    describe('run', function () {
        ...
        it('should give failing result if bucket cannot emit object events', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(2)
                expect(results[0].message).to.include('cannot emit object events')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
        ...
        it('should give passing result if bucket can emit object events', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(0)
                expect(results[0].message).to.include('can emit object events')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
