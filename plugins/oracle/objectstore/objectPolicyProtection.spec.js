...
var plugin = require('./objectPolicyProtection');
...
describe('objectPolicyProtection', function () {
    describe('run', function () {
        ...
        it('should give failing result if a policy does not have object store deletion protection', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
        ...
        it('should give passing result if a policy has object store deletion protection enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(0)
            ...
            const cache = createCache(
                ...
                [
                    {
                        "statements": [
                            "Allow group Developers to read objects in tenancy where target.user.name == 'OBJECT_DELETE'",
                        ],
                        "id": "ocid1.policy.oc1..aaaaaaaapil3afuz45oxyvd3u73otqbsj4atjdorao6nvfr3yjqumnniscka",
                        "compartmentId": "ocid1.tenancy.oc1..aaaaaaaao43aqdrzuacodg7ffqv2zeauftjyjkwhnbrugt44ympzeiblxx7q",
                        "name": "Developers",
