...
var plugin = require('./nfsPolicyProtection');
...
describe('nfsPolicyProtection', function () {
    describe('run', function () {
        ...
        it('should give failing result if a policy does not have file storage deletion protection', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
        ...
        it('should give passing result if a policy has file storage deletion protection enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0)
                expect(results[0].status).to.equal(0)
                expect(results[0].message).to.include('All policies have file storage delete protection enabled')
                expect(results[0].region).to.equal('us-ashburn-1')
                done()
            };
