...
var auth = require('./logCheckpointsEnabled');
    ...
    describe('run', function() {
        ...
        it('should give failing result if postgresql server has log_checkpoints disabled', function(done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('Log Checkpoints is disabled for the PostgreSQL Server configuration');
                expect(results[0].region).to.equal('eastus');
                done()
            };
        ...
        it('should give passing result if postgresql server has log_checkpoints enabled', function(done) {
            const callback = (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Log Checkpoints is enabled for the PostgreSQL Server configuration');
                expect(results[0].region).to.equal('eastus');
                done()
            };
