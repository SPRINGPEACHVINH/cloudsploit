...
var auth = require('./logDisconnectionsEnabled');
...
describe('logDisconnectionsEnabled', function() {
    describe('run', function() {
        ...
        it('should give failing result if postgresql server has log_disconnections disabled', function(done) {
            const callback = (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Disconnection logs is disabled for the PostgreSQL Server configuration');
                expect(results[0].region).to.equal('eastus');
                done()
            };
        ...
        it('should give passing result if postgresql server has log_disconnections enabled', function(done) {
            const callback = (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Disconnection logs is enabled for the PostgreSQL Server configuration');
                expect(results[0].region).to.equal('eastus');
                done()
            };
