...
var namespaceInfraEncryption = require('./namespaceInfraEncryption.js');
...
describe('namespaceInfraEncryption', function () {
    describe('run', function () {
        ...
        it('should give passing result if namespace has infrastructure level encryption enabled', function (done) {
            const cache = createCache([namespaces[1]], null);
            namespaceInfraEncryption.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Service Bus Namespace has infrastructure level encryption enabled');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });

        it('should give failing result if namespace does not have infrastructure level encryption enabled', function (done) {
            const cache = createCache([namespaces[0]], null);
            namespaceInfraEncryption.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Service Bus Namespace does not have infrastructure level encryption enabled');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
