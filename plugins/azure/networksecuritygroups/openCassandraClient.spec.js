...
var openCassandraClient = require('./openCassandraClient');
...
const networkSecurityGroups = [
    {
        ...
        "securityRules": [
            ...
            {
                "name": "AllowCassandraClient",
                "id": "/subscriptions/ab12c345-def7-890g-a1b2-28fc0d22117e/resourceGroups/test-rg/providers/Microsoft.Network/networkSecurityGroups/aadds-nsg/securityRules/AllowCassandraClient",
                "etag": "W/\"a1bb27cd-711f-4ede-b673-2fe8e7e07eee\"",
                "type": "Microsoft.Network/networkSecurityGroups/securityRules",
                "properties": {
                    "provisioningState": "Succeeded",
    ...
    },
    {
        ...
        "securityRules": [
        {
            ...
            "id": "/subscriptions/ab12c345-def7-890g-a1b2-28fc0d22117e/resourceGroups/test-rg/providers/Microsoft.Network/networkSecurityGroups/aadds-nsg/securityRules/AllowElasticsearch",
            "etag": "W/\"a1bb27cd-711f-4ede-b673-2fe8e7e07eee\"",
            "type": "Microsoft.Network/networkSecurityGroups/securityRules",
            "properties": {
                "provisioningState": "Succeeded",
...
describe('openCassandraClient', function() {
    describe('run', function() {
        ...
        it('should PASS if security group has cassandra client TCP port 9042 not open to public', function(done) {
        ...
        it('should FAIL if security group has cassandra client TCP port 9042 open to public', function(done) {
            const cache = createCache([networkSecurityGroups[1]]);
            openCassandraClient.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Security group')
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
