...
var server = require('./mysqlServerHasTags');
    ...
    describe('run', function() {
        ...
        it('should give failing result if MySQL Server does not have tags', function(done) {
            ...
            server.run(cache, {}, (err, results) => {
                ...
                expect(results[0].message).to.include('MySQL server does not have tags associated');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
        ...
        it('should give passing result if MySQL Server has tags', function(done) {
            ...
            server.run(cache, {}, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('MySQL server has tags associated');
                expect(results[0].region).to.equal('eastus');
                done();
            });
        });
