...
module.exports = {
    ...
    severity: 'High',
    description: 'Proactively check for RBAC Key Vault secrets expiry date and rotate them before expiry date is reached.',
        ...
        async.each(locations.vaults, function(location, rcb) {
            ...
            vaults.data.forEach(function(vault) {
                ...
                if (!secrets || secrets.err || !secrets.data) {
                    ...
                    helpers.addResult(results, 0, 'No Key Vault secrets found in RBAC vault', location, vault.id);
                } else {
                    secrets.data.forEach(function(secret) {
                        ...
                        if (!secret.attributes || !secret.attributes.enabled) {
                            helpers.addResult(results, 0, 'Secret in RBAC vault is not enabled', location, secretId);
                            ...
                            if (difference > config.key_vault_secret_expiry_fail) {
                                helpers.addResult(results, 0,
                                    `Secret in RBAC vault expires in ${difference} days`, location, secretId);
