...
var virtualMachinesDeallocateAlert = require("./virtualMachinesDeallocateAlert");
...
describe("virtualMachinesDeallocateAlert", function () {
  describe("run", function () {
    it("should give failing result if no activity log alerts found", function (done) {
        const cache = createCache(null, []);
        virtualMachinesDeallocateAlert.run(cache, {}, (err, results) => {
    ...
    it("should give failing result if Virtual Machines deallocate is not enabled", function (done) {
        const cache = createCache(null, [activityLogAlerts[0]]);
        virtualMachinesDeallocateAlert.run(cache, {}, (err, results) => {
            expect(results.length).to.equal(1);
            expect(results[0].status).to.equal(2);
            expect(results[0].message).to.include(
                "Log alert for Virtual Machines deallocate is not enabled"
            );
            expect(results[0].region).to.equal("global");
            done();
        });
    ...
    it("should give passing result if Virtual Machines deallocate is enabled", function (done) {
        const cache = createCache(null, [activityLogAlerts[1]]);
        virtualMachinesDeallocateAlert.run(cache, {}, (err, results) => {
            expect(results.length).to.equal(1);
            expect(results[0].status).to.equal(0);
            expect(results[0].message).to.include(
                "Log alert for Virtual Machines deallocate is enabled"
            );
            expect(results[0].region).to.equal("global");
            done();
        });
