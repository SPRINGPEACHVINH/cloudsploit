...
module.exports = {
    ...
    run: function(cache, settings, callback) {
        ...
        async.each(locations.activityLogAlerts, function(location, rcb) {
            ...
            if (!alertDeallocateEnabled) {
                helpers.addResult(results, 2,
                    `Log alert for ${text} deallocate is not enabled`, location, subscriptionId);
            } else {
                helpers.addResult(results, 0,
                    `Log alert for ${text} deallocate is enabled`, location, subscriptionId);
            }
