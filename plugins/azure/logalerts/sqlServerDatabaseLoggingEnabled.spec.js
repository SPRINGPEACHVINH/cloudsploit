...
var sqlServerDatabaseLoggingEnabled = require("./sqlServerDatabaseLoggingEnabled");
...
describe("sqlServerDatabaseLoggingEnabled", function () {
  describe("run", function () {
    ...
    it("should give failing and passing results if SQL Server Database delete is not enaled but write is enabled", function (done) {
        const cache = createCache(null, [activityLogAlerts[2]]);
        sqlServerDatabaseLoggingEnabled.run(cache, {}, (err, results) => {
            expect(results.length).to.equal(2);
            expect(results[0].status).to.equal(0);
            expect(results[0].message).to.include(
                "Log alert for SQL Server Database write is enabled"
            );
            expect(results[0].region).to.equal("global");
            expect(results[1].status).to.equal(2);
            expect(results[1].message).to.include(
                "Log Alert for SQL Server Database delete is not enabled"
            );
            expect(results[0].region).to.equal("global");
            done();
        });
    ...
    it("should give failing and passing results if SQL Server Database write is not enaled but delete is enabled", function (done) {
        const cache = createCache(null, [activityLogAlerts[3]]);
        sqlServerDatabaseLoggingEnabled.run(cache, {}, (err, results) => {
            expect(results.length).to.equal(2);
            expect(results[0].status).to.equal(0);
            expect(results[0].message).to.include(
                "Log alert for SQL Server Database delete is enabled"
            );
            expect(results[0].region).to.equal("global");
            expect(results[1].status).to.equal(2);
            expect(results[1].message).to.include(
                "Log alert for SQL Server Database write is not enabled"
            );
            expect(results[0].region).to.equal("global");
            done();
        });
