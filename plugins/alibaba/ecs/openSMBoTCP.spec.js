...
describe('openSMBoTCP', function () {
    describe('run', function () {
        it('should PASS if no public open ports found', function (done) {
            ...
            openSMBoTCP.run(cache, { china: true }, (err, results) => {
        ...
        it('should FAIL if security group has SMBoTCP TCP 445 port open to public', function (done) {
            const cache = createCache(describeSecurityGroups, describeSecurityGroupAttribute[1]);
            openSMBoTCP.run(cache, { china: true }, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('has SMBoTCP:TCP:445 open to 0.0.0.0/0');
                expect(results[0].region).to.equal('cn-hangzhou');
                done();
            });
        });
