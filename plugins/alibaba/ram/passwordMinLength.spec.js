...
describe('passwordMinLength', function () {
    describe('run', function () {
        it('should FAIL if RAM password security policy does not require minimum length of 14 or greater', function (done) {
            ...
            passwordMinLength.run(cache, { ram_password_min_length: '15' }, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('RAM password security policy requires minimum length of 8 which is less than desired limit of 15');
        ...
        it('should PASS if RAM password security policy require minimum length of 14 or greater', function (done) {
            ...
            passwordMinLength.run(cache, { china: true }, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('RAM password security policy requires minimum length of 15 which is equal to or greater than desired limit of 14');
                expect(results[0].region).to.equal('cn-hangzhou');
                done();
            });
        });
