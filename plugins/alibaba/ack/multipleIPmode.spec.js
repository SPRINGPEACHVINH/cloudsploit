...
describe('ENImultipleIPmode', function () {
    describe('run', function () {
        it('should FAIL if Cluster does not have NetworkPolicy Terway enabled', function (done) {
            const cache = createCache([describeClusters[1]]);
            ENImultipleIPmode.run(cache, { china: true }, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Cluster does not have ENI Multiple IP Mode enabled');
                expect(results[0].region).to.equal('cn-hangzhou');
                done();
            });
        });

        it('should PASS if Cluster has NetworkPolicy Terway enabled', function (done) {
            const cache = createCache([describeClusters[0]]);
            ENImultipleIPmode.run(cache, { china: true }, (err, results) => {
                expect(results.length).to.equal(1);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Cluster has ENI Multiple IP Mode enabled');
                expect(results[0].region).to.equal('cn-hangzhou');
                done();
            });
        });
