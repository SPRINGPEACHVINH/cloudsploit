...
var plugin = require('./autoNodeRepairEnabled');
...
describe('autoNodeRepairEnabled', function () {
    describe('run', function () {
        ...
        it('should give passing result if auto node repair is enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Auto repair is enabled for all node pools');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if auto node repair is disabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Auto repair is disabled for these node pools');
                expect(results[0].region).to.equal('global');
                done()
            };
