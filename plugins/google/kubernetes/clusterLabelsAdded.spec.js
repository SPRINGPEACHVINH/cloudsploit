...
var plugin = require('./clusterLabelsAdded');
...
describe('clusterLabelsAdded', function () {
    describe('run', function () {
        ...
        it('should give passing result if labels have been added to the cluster', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('labels found for the cluster');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if no labels have been added to the cluster', function (done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('The cluster does not have any labels added.');
                expect(results[0].region).to.equal('global');
                done()
            };
