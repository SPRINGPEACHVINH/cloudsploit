...
var plugin = require('./binaryAuthorizationEnabled');
...
describe('binaryAuthorizationEnabled', function () {
    describe('run', function () {
        ...
        it('should give passing result if binary authorization is enabled for the cluster', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('is enabled on the cluster');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if binary authorization is not enabled on the cluster', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('is not enabled on the cluster');
                expect(results[0].region).to.equal('global');
                done()
            };
