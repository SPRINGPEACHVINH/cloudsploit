...
var plugin = require('./nodeEncryption');
        ...
        "nodePools": [
            {
                "name": "pool-1",
                "config": {
                    "bootDiskKmsKey": 'projects/test-dev-aqua/locations/global/keyRings/test-kr/cryptoKeys/test-key-2'
                },
                "initialNodeCount": 3,
                "locations": [
                    "us-central1-a"
...
describe('nodeEncryption', function () {
    describe('run', function () {
        ...
        it('should give passing result if node encryption level is equal to or greater than desired level', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
        ...
        it('should give failing result if node encryption level is less than desired level', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
        ...
        it('should give failing result if node encryption level key is not found', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('These node pools do not have the');
