...
var plugin = require('./legacyAuthorizationDisabled');
...
describe('legacyAuthorizationDisabled', function () {
    describe('run', function () {
        ...
        it('should give passing result if Legacy Authorization is disabled on the cluster', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Legacy Authorization is disabled on the cluster');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if Legacy Authorization is enabled on the cluster', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Legacy Authorization is enabled on the cluster');
                expect(results[0].region).to.equal('global');
                done()
            };
