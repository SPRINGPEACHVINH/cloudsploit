...
var plugin = require('./basicAuthenticationDisabled');
...
describe('basicAuthenticationDisabled', function () {
    describe('run', function () {
        ...
        it('should give passing result if Basic authentication is disabled on the cluster', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Basic authentication is disabled on the cluster');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if Basic authentication is enabled on the cluster', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Basic authentication is enabled on the cluster');
                expect(results[0].region).to.equal('global');
                done()
            };
                ...
                [
                    {
                        ...
                        "masterAuth": {
                            ...
                            "username" : "John@cloud.com",
                            "password" : "johncloudpassword"
                        },
                        "loggingService": "none",
