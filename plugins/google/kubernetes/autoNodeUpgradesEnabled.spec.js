...
var plugin = require('./autoNodeUpgradesEnabled');
...
describe('autoNodeUpgradesEnabled', function () {
    describe('run', function () {
        ...
        it('should give passing result if Auto upgrades are enabled for the node pool of the cluster', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
        ...
        it('should give failing result if Auto upgrades are disabled for the node pool of the cluster', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
