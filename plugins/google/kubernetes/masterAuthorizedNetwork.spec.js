...
var plugin = require('./masterAuthorizedNetwork');
...
describe('masterAuthorizedNetwork', function () {
    describe('run', function () {
        ...
        it('should give passing result if Master Authorized Network is enabled on the Kubernetes cluster', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Master Authorized Network is enabled on the Kubernetes cluster');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if Master Authorized Network is disabled on the Kubernetes cluster', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Master Authorized Network is disabled on the Kubernetes cluster');
                expect(results[0].region).to.equal('global');
                done()
            };
