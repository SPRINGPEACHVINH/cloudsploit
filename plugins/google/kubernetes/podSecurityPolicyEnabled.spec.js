...
var plugin = require('./podSecurityPolicyEnabled');
...
describe('podSecurityPolicyEnabled', function () {
    describe('run', function () {
        ...
        it('should give passing result if Pod security policy config is enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Pod security policy config is enabled');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if Pod security policy config is disabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Pod security policy config is disabled');
                expect(results[0].region).to.equal('global');
                done()
            };
