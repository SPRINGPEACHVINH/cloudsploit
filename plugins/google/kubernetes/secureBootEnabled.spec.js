...
var plugin = require('./secureBootEnabled');
...
describe('secureBootEnabled', function () {
    describe('run', function () {
        ...
        it('should give passing result if secure boot is enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Secure Boot is enabled for all node pools');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if secure boot is disabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Secure Boot is disabled for these node pools');
                expect(results[0].region).to.equal('global');
                done()
            };
