...
describe('storagePermissionsLogging', function () {
    describe('run', function () {
        ...
        it('should give passing result if log alert for storage permission changes is enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
        ...
        it('should give failing result if log alert for storage permission changes is not enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Log metric for storage permission changes not found');
                expect(results[0].region).to.equal('global');
                done()
            };
