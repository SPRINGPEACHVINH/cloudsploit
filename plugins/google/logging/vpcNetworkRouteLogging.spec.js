...
describe('vpcNetworkRouteLogging', function () {
    describe('run', function () {
        ...
        it('should give passing result if log alert for vpc network route changes are enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Log alert for VPC network route changes is enabled');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give passing result if log alert for vpc network route changes are not enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
