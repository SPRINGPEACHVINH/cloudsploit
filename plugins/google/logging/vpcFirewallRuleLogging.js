    ...
    run: function(cache, settings, callback) {
        ...
        async.each(regions.alertPolicies, function(region, rcb){
            ...
            if (disabled) {
                ...
                if (!conditionFound) {
                    helpers.addResult(results, 2, 'Log alert for firewall rule changes not found', region);
                }
            } else {
                helpers.addResult(results, 2, 'Log metric for firewall rule changes not found', region);
            }
