...
var plugin = require('./projectOwnershipLogging');
...
describe('projectOwnershipLogging', function () {
    describe('run', function () {
        ...
        it('should give failing result if log metric for project ownership changes is disbled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Log metric for project ownership changes is disbled');
                expect(results[0].region).to.equal('global');
                done()
            };
                ...
                [
                    {
                        ...
                        "conditions": [
                            {
                                "conditionThreshold": {
                                    "filter": "metric.type=\"logging.googleapis.com/user/ProjectOwnershipAssignmentsChanges\" resource.type=\"metric\"",
        ...
        it('should give passing result if log alert for project ownership changes are enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
        ...
        it('should give failing result if log alert for project ownership changes are not enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
