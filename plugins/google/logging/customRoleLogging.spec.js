...
var plugin = require('./customRoleLogging');
...
describe('customRoleLogging', function () {
    describe('run', function () {
        ...
        it('should give passing result if log alerts for custom role changes are enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Log alert for custom role changes is enabled');
                expect(results[0].region).to.equal('global');
                done()
            };
                ...
                [
                    {
                        ...
                        "metricDescriptor": {
                            "name": "projects/rosy-red-12345/metricDescriptors/logging.googleapis.com/user/CustomRoleLogging",
                            "metricKind": "DELTA",
                            "valueType": "INT64",
                            "unit": "1",
                            "description": "Ensure log metric filter and alerts exists for Project Ownership assignments/changes",
                            "type": "logging.googleapis.com/user/CustomRoleLogging"
                        },
                ...
                [
                    {
                        ...
                        "conditions": [
                            {
                                "conditionThreshold": {
                                    "filter": "metric.type=\"logging.googleapis.com/user/CustomRoleLogging\" resource.type=\"metric\"",
                                ...
                                "displayName": "logging/user/CustomRoleLogging",
        ...
        it('should give failing result if log alerts for custom role changes are not enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
