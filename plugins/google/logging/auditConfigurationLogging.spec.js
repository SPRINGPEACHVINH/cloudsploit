...
var plugin = require('./auditConfigurationLogging');
...
describe('auditConfigurationLogging', function () {
    describe('run', function () {
        ...
        it('should give passing result if log alert for audit configuration changes are enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Log alert for audit configuration changes is enabled');
                expect(results[0].region).to.equal('global');
                done()
            };
                ...
                [
                    {
                        ...
                        "metricDescriptor": {
                            ...
                            "unit": "1",
                            "description": "Ensure log metric filter and alerts exists for Project Ownership assignments/changes",
                            "type": "logging.googleapis.com/user/auditConfigDeltas"
                        },
                ...
                [
                    {
                        ...
                        "conditions": [
                            {
                                "conditionThreshold": {
                                    "filter": "metric.type=\"logging.googleapis.com/user/auditConfigDeltas\" resource.type=\"metric\"",
                                ...
                                "displayName": "logging/user/auditConfigDeltas",
        ...
        it('should give failing result if log alert for audit configuration changes are not enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Log metric for audit configuration changes not found');
                expect(results[0].region).to.equal('global');
                done()
            };
