...
var plugin = require('./serviceAccountUser');
...
describe('serviceAccountUser', function () {
    describe('run', function () {
        ...
        it('should give passing result if no user has the Service Account User role', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('No accounts have service account user roles');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if a service account has editor privileges', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('The account has a service account user role');
                expect(results[0].region).to.equal('global');
                done()
            };
