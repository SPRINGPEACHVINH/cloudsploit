...
var plugin = require('./bigtableAdmin');
...
describe('bigtableAdmin', function () {
    describe('run', function () {
        ...
        it('should give passing result if no user has the bigtable admin role', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('No accounts have the pre-defined Bigtable Admin role');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if the user has the bigtable admin role', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
