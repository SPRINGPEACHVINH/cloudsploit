...
var plugin = require('./ingressAllTrafficDisabled');
...
describe('ingressAllTrafficDisabled', function () {
    describe('run', function () {
        ...
        it('should give passing result if google cloud function is configured to allow internal and GCLB traffic', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
