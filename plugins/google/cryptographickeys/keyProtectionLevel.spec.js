...
var plugin = require('./keyProtectionLevel');
...
describe('keyProtectionLevel', function () {
    describe('run', function () {
        ...
        it('should give passing result if key protection level greater than or equal to desired protection level', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
            ...
            plugin.run(cache, { kms_crypto_keys_protection_level: 'cloudhsm' }, callback);
        });

        it('should give failing result if key protection level is which is less than desired protection level', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
            ...
            plugin.run(cache, { kms_crypto_keys_protection_level: 'cloudcmek' }, callback);
        })
    })
});