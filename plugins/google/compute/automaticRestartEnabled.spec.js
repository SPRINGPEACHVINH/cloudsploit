...
var plugin = require('./automaticRestartEnabled');
...
describe('automaticRestartEnabled', function () {
    describe('run', function () {
        ...
        it('should fail if Automatic Restart is disabled for the instance', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Automatic Restart is disabled for the instance');
                expect(results[0].region).to.equal('us-central1');
                done()
            };
        ...
        it('should pass if Automatic Restart is enabled for the instance', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.equal('Automatic Restart is enabled for the instance');
                expect(results[0].region).to.equal('us-central1');
                done()
            };
