...
var plugin = require('./sqlServerUserConnections');
...
describe('sqlServerUserConnections', function () {
    describe('run', function () {
        ...
        it('should give passing result if sql instance has user connections flag set to desired value', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('SQL instance has "user connections" flag set to allow maximum number of connections');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give passing result if sql instance has user connections flag set to desired value', function (done) {
            ...
            plugin.run(cache, {min_user_connections: 50}, callback);
        });
        ...
        it('should give failing result if sql instance does not have user connections flag set to desired value', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
