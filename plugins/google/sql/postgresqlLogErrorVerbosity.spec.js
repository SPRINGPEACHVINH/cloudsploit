...
var plugin = require('./postgresqlLogErrorVerbosity');
...
describe('postgresqlLogErrorVerbosity', function () {
    describe('run', function () {
        ...
        it('should give passing result if sql instance has log_error_verbosity flag set to default', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
        ...
        it('should give passing result if sql instance has log_error_verbosity flag set to default or stricter', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('which is greater than or equal to');
            ...
            plugin.run(cache, { log_error_verbosity: 'FATAL' }, callback);
        });

        it('should give failing result if sql instance does not have log_error_verbosity flag set to default or stricter', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
