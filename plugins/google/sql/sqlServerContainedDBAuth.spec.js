...
var plugin = require('./sqlServerContainedDBAuth');
...
describe('sqlServerContainedDBAuth', function () {
    describe('run', function () {
        ...
        it('should give passing result if sql instance does not have contained database authentication flag enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
        ...
        it('should give failing result if sql instance has contained database authentication flag enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
