...
var plugin = require('./postgresqlLogMinDuration');
...
describe('postgresqlLogMinDuration', function () {
    describe('run', function () {
        ...
        it('should give passing result if sql instances does not have log_min_duration_statement flag', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('SQL instance does not have log_min_duration_statement flag enabled');
