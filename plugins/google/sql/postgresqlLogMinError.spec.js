...
var plugin = require('./postgresqlLogMinError');
...
describe('postgresqlLogMinError', function () {
    describe('run', function () {
        ...
        it('should give passing result if sql instances have log_min_error_statement flag set to Error', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
        ...
        it('should give failing result if sql instances does not have log_min_error_statement flag set to Error', function (done) {
            const callback = (err, results) => {
                ...
                expect(results[0].message).to.include('SQL instance does not have log_min_error_statement flag set to Error');
                expect(results[0].region).to.equal('global');
                done()
            };
