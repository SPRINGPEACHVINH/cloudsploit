...
var plugin = require('./postgresqlLogTempFiles');
...
describe('postgresqlLogTempFiles', function () {
    describe('run', function () {
        ...
        it('should give passing result if sql instances have log_temp_files flag enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('SQL instance have log_temp_files flag enabled');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give failing result if sql instances does not have log_temp_files flag enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('SQL instance does not have log_temp_files flag enabled');
                expect(results[0].region).to.equal('global');
                done()
            };
