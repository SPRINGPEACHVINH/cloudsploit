...
var plugin = require('./legacyNetworksExist');
...
describe('legacyNetworksExist', function () {
    describe('run', function () {
        ...
        it('should give failing result if vpc network is in legacy mode', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('VPC Network is in legacy mode');
                expect(results[0].region).to.equal('global');
                done()
            };
            const cache = createCache(
        ...
        it('should give passing result if vpc network is not in legacy mode', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('VPC Network is not in legacy mode');
                expect(results[0].region).to.equal('global');
                done()
            };
            const cache = createCache(
