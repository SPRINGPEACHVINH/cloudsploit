...
var plugin = require('./clbLoggingEnabled');
...
describe('clbLoggingEnabled', function () {
    describe('run', function () {
        ...
        it('should give passing result if logging is enabled for the backend service', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
                expect(results[0].message).to.include('Logging is enabled for the backend service');
                expect(results[0].region).to.equal('global');
                done()
            };
        ...
        it('should give passing result if logging is disabled for the backend service', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(2);
                expect(results[0].message).to.include('Logging is disabled for the backend service');
                expect(results[0].region).to.equal('global');
                done()
            };
