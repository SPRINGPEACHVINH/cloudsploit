...
var plugin = require('./uniformBucketLevelAccess');
...
describe('uniformBucketLevelAccess', function () {
    describe('run', function () {
        ...
        it('should give passing result if uniform bucket level access is enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
        ...
        it('should give failing result if uniform bucket level access is disabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
