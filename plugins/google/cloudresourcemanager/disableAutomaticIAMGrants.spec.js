...
var plugin = require('./disableAutomaticIAMGrants');
...
describe('disableAutomaticIAMGrants', function () {
    describe('run', function () {
        ...
        it('should give passing result if automatic IAM Grant is disabled for service accounts', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
        ...
        it('should give failing result if automatic IAM Grant is not disabled for service accounts', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
