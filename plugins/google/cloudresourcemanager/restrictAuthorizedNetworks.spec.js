...
var plugin = require('./restrictAuthorizedNetworks');
...
describe('restrictAuthorizedNetworks', function () {
    describe('run', function () {
        ...
        it('should give passing result if restrict authorized networks constraint is enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
        ...
        it('should give failing result if restrict authorized networks constraint is disabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
