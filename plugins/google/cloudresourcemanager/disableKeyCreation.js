...
module.exports = {
    title: 'Disable Service Account Key Creation',
    ...
    recommended_action: 'Ensure that "Disable Service Account Key Creation" constraint is enforced at the organization level.',
    apis: ['organizations:list', 'organizations:listOrgPolicies'],
    remediation_min_version: '202207280432',
    remediation_description: 'The "Disable Service Account Key Creation" constraint will be enforced at the organization level.',
    ...
    run: function(cache, settings, callback) {
        ...
        helpers.checkOrgPolicy(orgPolicies, 'iam.disableServiceAccountKeyCreation', 'booleanPolicy', true, false, 'Disable Service Account Key Creation', results, organization);
    ...
    remediate: function(config, cache, settings, resource, callback) {
        ...
        var pluginName = 'disableKeyCreation';
 
        var putCall = this.actions.remediate;

        helpers.remediateOrgPolicy(config, 'constraints/iam.disableServiceAccountKeyCreation', 'booleanPolicy', true, resource, remediation_file, putCall, pluginName, function(err, action) {
