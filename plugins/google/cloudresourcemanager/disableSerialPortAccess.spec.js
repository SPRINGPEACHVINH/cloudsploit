...
var plugin = require('./disableSerialPortAccess');
...
describe('disableSerialPortAccess', function () {
    describe('run', function () {
        ...
        it('should give passing result if serial port access is disabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
        ...
        it('should give failing result if serial port access is not disabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
