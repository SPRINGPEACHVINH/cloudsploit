...
var plugin = require('./restrictVPNPeerIPs');
...
describe('restrictVPNPeerIPs', function () {
    describe('run', function () {
        ...
        it('should give passing result if restrict vpn peer IPs is enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
                expect(results[0].status).to.equal(0);
        ...
        it('should give failing result if restrict vpn peer IPs is not enabled', function (done) {
            const callback = (err, results) => {
                expect(results.length).to.be.above(0);
