...
const serviceNameMapping = {
  CodeStar: "CodeStarconnections", // CodeStar đã được thay thế bởi CodeStarconnections
  // Thêm các mapping khác nếu cần
};

// Function to get correct service name
...
var collect = function (AWSConfig, settings, callback) {
  ...
  AWSEC2.describeRegions(params, function (err, accountRegions) {
    if (err) {
      console.log(
        `[INFO][REGIONS] Could not load all regions from EC2: ${JSON.stringify(
          err
        )}`
      );
    } else {
    ...
    async.eachOfLimit(
      helpers.calls,
      10,
      function (call, service, serviceCb) {
        ...
        async.eachOfLimit(
          call,
          15,
          function (callObj, callKey, callCb) {
            if (
              settings.api_calls &&
              settings.api_calls.indexOf(serviceName + ":" + callKey) === -1
            )
              return callCb();
            ...
            async.eachLimit(
              callRegions,
              helpers.MAX_REGIONS_AT_A_TIME,
              function (region, regionCb) {
                if (
                ...
                if (callObj.override) {
                  collectors[serviceLower][callKey](
                    LocalAWSConfig,
                    collection,
                    retries,
                    function () {
                  ...
                  try {
                    let actualServiceName = getCorrectServiceName(serviceName);

                    if (!AWS[actualServiceName]) {
                      // Try fallback options
                      if (serviceName === "CodeStar") {
                        // CodeStar has been discontinued, check for alternatives
                        const alternatives = [
                          "CodeStarconnections",
                          "CodeCommit",
                          "CodeBuild",
                        ...
                        for (let alt of alternatives) {
                          if (AWS[alt]) {
                            console.log(
                              `INFO: Using ${alt} as alternative for CodeStar`
                            );
                            actualServiceName = alt;
                        ...
                        if (!foundAlternative) {
                          console.warn(
                            `WARNING: CodeStar and alternatives not available, skipping...`
                          );
                          return regionCb();
                        }
                      } else {
                        console.warn(
                          `WARNING: AWS service '${serviceName}' not available, skipping...`
                        );
                        return regionCb();
                      }
                    }

                    var executor = debugMode
                      ? AWSXRay.captureAWSClient(
                          new AWS[actualServiceName](LocalAWSConfig)
                        )
                      : new AWS[actualServiceName](LocalAWSConfig);
                    // var executor = debugMode
                    //   ? AWSXRay.captureAWSClient(
                    //       new AWS[serviceName](LocalAWSConfig)
                    //     )
                    //   : new AWS[serviceName](LocalAWSConfig);
                  } catch (serviceError) {
                    console.error(
                      `ERROR: Failed to create AWS service '${serviceName}': ${serviceError.message}`
                    );
                    return regionCb();
                  }
                  ...
                  var executorCb = function (err, data) {
                    ...
                    var dataToAdd = callObj.secondProperty
                      ? data[callObj.property][callObj.secondProperty]
                      : data[callObj.property]
                      ? data[callObj.property]
                      : data;

                    if (paginating) {
                      collection[serviceLower][callKey][region].data =
                        collection[serviceLower][callKey][region].data.concat(
                          dataToAdd
                        );
                    } else {
                      collection[serviceLower][callKey][region].data =
                    ...
                    if (settings.paginate && nextToken && data[nextToken]) {
                      paginating = true;
                      var paginateProp = callObj.paginateReqProp
                        ? callObj.paginateReqProp
                        : nextToken;
                  ...
                  function execute(nextTokens) {
                    ...
                    var localParams = JSON.parse(
                      JSON.stringify(callObj.params || {})
                    );
                      ...
                      async.retry(
                        {
                          ...
                          interval: function (retryCount) {
                            ...
                            let timestamp = parseInt(
                              new Date().getTime().toString().slice(-1)
                            );
                            let retry_temp = Math.min(
                              apiRetryCap,
                              apiRetryBackoff *
                                (retryExponential + timestamp) ** retryCount
                            );
                            let retry_seconds = Math.round(
                              retry_temp / retryLeveler +
                                Math.random(0, retry_temp) * 5000
                            );

                            console.log(
                              `Trying ${callKey} again in: ${
                                retry_seconds / 1000
                              } seconds`
                            );
                            retries.push({
      ...
      function () {
        ...
        async.eachSeries(
          helpers.postcalls,
          function (postcallObj, postcallCb) {
            async.eachOfLimit(
              postcallObj,
              10,
              function (serviceObj, service, serviceCb) {
                var serviceName = service;
                var serviceLower = service.toLowerCase();
                var serviceIntegration = {
                  enabled:
                    postcallObj &&
                    postcallObj[serviceName] &&
                    postcallObj[serviceName].sendIntegration &&
                    postcallObj[serviceName].sendIntegration.enabled
                      ? true
                      : false,
                  sendLast:
                    postcallObj &&
                    postcallObj[serviceName] &&
                    postcallObj[serviceName].sendIntegration &&
                    postcallObj[serviceName].sendIntegration.sendLast
                      ? true
                      : false,
                };
                ...
                async.eachOfLimit(
                  ...
                  function (callObj, callKey, callCb) {
                    ...
                    async.eachLimit(
                      ...
                      function (region, regionCb) {
                        ...
                        if (
                          callObj.reliesOnService &&
                          !collection[callObj.reliesOnService]
                        )
                          return regionCb();

                        if (
                          callObj.reliesOnCall &&
                          (!collection[callObj.reliesOnService] ||
                            !collection[callObj.reliesOnService][
                              callObj.reliesOnCall
                            ] ||
                        ...
                        if (callObj.signatureVersion)
                          LocalAWSConfig.signatureVersion =
                            callObj.signatureVersion;
                        ...
                        if (callObj.override) {
                          ...
                          if (
                            !collection[callObj.reliesOnService][
                              callObj.reliesOnCall
                            ][LocalAWSConfig.region] ||
                            !collection[callObj.reliesOnService][
                              callObj.reliesOnCall
                            ][LocalAWSConfig.region].data
                          ) {
                            return regionCb();
                          }
                          ...
                          async.eachLimit(
                            ...
                            function (dep, depCb) {
                              collection[serviceLower][callKey][
                                LocalAWSConfig.region
                              ][dep[callObj.filterValue]] = {};

                              var filter = {};
                              filter[callObj.filterKey] =
                                dep[callObj.filterValue];
                              ...
                              async.retry(
                                {
                                  ...
                                  errorFilter: function (err) {
                                    return helpers.collectRateError(
                                      err,
                                      rateError
                                    );
                                  },
                                ...
                                function (cb) {
                                  executor[callKey](
                                    ...
                                    function (err, data) {
                                      if (
                                        helpers.collectRateError(err, rateError)
                                      ) {
                                        return cb(err);
                                      } else if (err) {
                                        collection[serviceLower][callKey][
                                          LocalAWSConfig.region
                                        ][dep[callObj.filterValue]].err = err;
                                        helpers.logError(
                                          serviceLower,
                                          callKey,
                                          region,
                                        ...
                                        collection[serviceLower][callKey][
                                          LocalAWSConfig.region
                                        ][dep[callObj.filterValue]].data = data;
                                        return cb();
                                      }
                                    }
                                  );
                  ...
                  function () {
                    if (
                      ...
                      try {
                        collectData.processIntegration(
                          serviceName,
                          settings,
                          collection,
                          helpers.calls,
          ...
          function () {
            if (settings.identifier) {
              async.each(
                helpers.integrationSendLast,
                function (serv, cb) {
                  settings.identifier.service = serv.toLowerCase();

                  if (
                    collection[serv.toLowerCase()] &&
                    ...
                    try {
                      collectData.processIntegration(
                        ...
                        function () {
                          console.log(
                            `Integration for service ${serv} processed.`
                          );
                          cb();
                        }
                      );
