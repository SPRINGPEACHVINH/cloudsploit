...
module.exports = function(AWSConfig, collection, retries, callback) {
    ...
    async.eachLimit(collection.guardduty.listDetectors[AWSConfig.region].data, 15, function(detectorId, cb) {
        ...
        helpers.makeCustomCollectorCall(guardduty, 'listPublishingDestinations', params, retries, null, null, null, function(err, data) {
            if (err) {
                collection.guardduty.listPublishingDestinations[AWSConfig.region][detectorId].err = err;
            }
            if (data) collection.guardduty.listPublishingDestinations[AWSConfig.region][detectorId].data = data;
        });
